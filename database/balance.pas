...
function getbalance(id:integer;nomencl:string;date:tdatetime):real;overload;
function getbalance(id:integer;nomencl:string;year,quarter:integer):real;overload;
...
begin
  ...
  dm.query.sql.text:='SELECT SUM(price) AS BALANCE'+#13+
                     ...
                     'WHERE (nomencl='+quotedstr(nomencl)+')'+
                     'AND (id<>'+quotedstr(inttostr(id))+')'+
                     'AND datepart(q,subdate)='+inttostr(quarter)+#13+
                     'AND year(subdate)='+inttostr(year)+#13+
                     'AND (report<>1)';
  dm.query.open;
  dm.query.first;
  if dm.query.recordcount>0 then
